...
public class SearchUserActivity extends BaseActivity {
			...
			QBUsers.getUsersByEmails(usersEmails, pagedRequestBuilder, new QBEntityCallbackImpl<ArrayList<QBUser>>() {
			    ...
			    public void onSuccess(ArrayList<QBUser> users, Bundle params) {
			    	if (users.size() == 0){
			    		...
			    		UserListAdapter userAdapter = new UserListAdapter(context, R.layout.list_item_invite_user, users);
	...
	public void invite(View v){
		...
		case R.id.invite_user:
		    ...
		    ArrayList<Integer> members = DataHolder.getDataHolder().getSignInUserGroup(position).getMembersId();
